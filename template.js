/*
 * The internal data structure used throughout afSlingshot.Model for data is
 * {
 *   filename(key): [
 *    {
 *      directive: directive,
...
afSlingshot.Model.prototype._upload = function upload(
    file,
    directive,
    uploaderCallback
  ) {
...
afSlingshot.Model.prototype._fileFromUploader = function(u) {
  var f = {
    src: u.url(),
...
afSlingshot.Model.prototype.add = function(fsFiles) {
  ...
  _.each(fsFiles, function(fsFile) {
    ...
    _.each(self._config.directives, function(directive) {
      ...
      var u = self._upload(fsFile, directive, function(err, downloadUrl) {
        ...
        file.setProp(
          directive.name,
          'key',
          _.findWhere(u.instructions.postData, {name: 'key'}).value
        );
      });
    ...
    files[ fsFile.name ] = new afSlingshot.FileRecord(
      fileDirectives,
      self._config
    );
  });
